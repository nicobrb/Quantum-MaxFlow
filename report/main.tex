...
Come visto nella sezione \ref{lp-formulation} il problema \emph{Min-Cut} è espresso come:
\begin{center}
\begin{table}[H]
    \centering
    \begin{tabular}{c|c}
        & soggetto a $\pi_t - \pi_s - s_1 -1 = 0$ \\
...
Nel quale i vincoli sono stati trasformati in uguaglianze tramite l'aggiunta di apposite variabili di slack, assicurandoci di eguagliare lo zero nella parte destra dell'equazione.
...
Nella riformulazione si useranno le variabili $y_i^j$, dove l'indice $i$ indica il vincolo di riferimento e l'indice $j$ rappresenta la potenza di 2 corrispondente. Pertanto, la variabile composta, moltiplicata per il suo coefficiente, sarà: $2^jy_i^j$.
